 ...
 *
 * 원본에서 이 사이트에 맞게 바꾼 것 네 가지.
 *
 *  1. GRAVITY   원본 속도장에는 아래로 미는 항이 없어서 염료가 사방으로 똑같이 퍼진다.
 *               그건 물이나 담배연기지 드라이아이스가 아니다.
 *               vorticity 패스 끝에 중력항을 한 줄 넣어서 연기가 아래로 깔리게 했다.
 *  2. 색        무지개를 끄고 frost 단색. BRIGHTNESS 로 세기를 따로 조절한다
 *               (원본은 0.15 로 고정이라 어두운 배경에서 거의 안 보인다).
 *  3. z-index   원본은 50 이라 nav(40) 위로 올라온다. 13 으로 내려서
 *               상세(12) 위 · nav(40) 아래에 놓았다.
 *  4. 가드      WebGL 이 없거나 prefers-reduced-motion 이면 아예 안 켠다.
 *               원본은 gl 이 null 이면 그대로 터진다.
 *
...
function SplashCursor({
  ...
  RAINBOW_MODE = false,
  COLOR = '#AFC2C8',
  /* 아래로 미는 힘. CO2 는 공기보다 무겁다. 0 이면 원본과 같다. */
  GRAVITY = 55,
  BRIGHTNESS = 0.2,
  Z_INDEX = 13,
}) {
  ...
  useEffect(() => {
    ...
    function getWebGLContext(c) {
      ...
      const supportRender = (internalFormat, format, type) => {
      ...
      const getSupportedFormat = (internalFormat, format, type) => {
        if (!supportRender(internalFormat, format, type)) {
    ...
    /* WebGL 이 없는 환경에서 원본은 여기서 그대로 터진다 */
    ...
    /* ── 여기가 원본과 다른 곳 ──
       uGravity 만큼 매 프레임 속도를 아래로 민다.
       vUv 의 y 는 위가 1 이므로 빼면 아래로 간다.
       이 한 줄이 "떠다니는 연기"와 "가라앉는 연기"를 가른다. */
    const vorticityShader = compileShader(gl.FRAGMENT_SHADER, `
      precision highp float; precision highp sampler2D;
      varying vec2 vUv; varying vec2 vL; varying vec2 vR; varying vec2 vT; varying vec2 vB;
      uniform sampler2D uVelocity; uniform sampler2D uCurl;
      uniform float curl; uniform float dt; uniform float uGravity;
      void main () {
    ...
    function step(dt) {
      ...
      gl.uniform1f(vorticityProgram.uniforms.uGravity, config.GRAVITY);
      blit(velocity.write);
      velocity.swap();
    ...
    function generateColor() {
      ...
      c.r *= config.BRIGHTNESS; c.g *= config.BRIGHTNESS; c.b *= config.BRIGHTNESS;
      return c;
    }
